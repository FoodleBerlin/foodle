...
describe('Foodle-website', () => {
  ...
  context('Authentication', () => {
    it('redirects to google on login', () => {
      cy.get('button').contains('Login').click();
      cy.get('button').contains('Continue with google').click();
      cy.url().should('include', 'accounts.google.com');
    });
    it('redirects to facebook on login', () => {
        cy.get('button').contains('Login').click();
        cy.get('button').contains('Continue with google').click();
        cy.url().should('include', 'accounts.google.com');
      });
  });
  const protectedRoutes = ['/create', '/account'];
  context('Athorization', () => {
    protectedRoutes.forEach((protectedRoute) => {
      it('protects ' + protectedRoute + ' from unautheticated users', () => {
        cy.visit('http://localhost:3000' + protectedRoute);
        cy.url().should('not.include', protectedRoute);
      });
    });
  });
});
...
//         cy.get('h2').contains('What kind of property do you own?')
//     }
// })
