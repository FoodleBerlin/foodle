...
const parse = (defaults, pattern) => {
    ...
    return patterns.map((p) => (0, exports.parseOne)(defaults, p));
};
exports.parse = parse;
/**
 ...
 */
const parseOne = (criteriaDefaults, pattern) => {
    // todo maybe level default should be wildcard instead...
    const originalInput = pattern;
    const level = { ...criteriaDefaults.level };
    const path = { value: ``, descendants: false };
    if (pattern === ``) {
    ...
    if (levelm) {
        if (levelm.type === `wildcard`) {
            ...
            if (levelm.dir === `+`)
                ...
                (0, utils_cjs_1.casesHandled)(levelm.dir);
        }
    }
    /**
     * Check For Errors
    ...
    if (path.value !== null) {
        const invalidPathPartNames = path.value
            ...
            .slice(1) // root
            .filter((pathPart) => !data_cjs_1.validPathSegmentNameRegex.exec(pathPart));
        if (invalidPathPartNames.length) {
            return createInvalidPattern(originalInput, `Path segment names must only contain ${String(data_cjs_1.validPathSegmentNameRegex)}.`);
        }
    }
...
const comp = (kind, a, b) => {
    ...
    if (kind === `lte`)
        return a <= b;
    (0, utils_cjs_1.casesHandled)(kind);
 ...
 */
const parseUnsafe = (defaults, pattern) => {
    return (0, exports.parse)(defaults, pattern).map((value) => {
...
const createInvalidPattern = (pattern, hint) => {
    return (0, utils_cjs_1.createContextualError)(`Invalid filter pattern: "${pattern}${hint ? `. ${hint}` : ``}"`, {
        pattern,
        hint
    });
};
 ...
 */
const renderSyntaxManual = () => {
    const m = chalk_cjs_1.chalk.magenta;
    const b = chalk_cjs_1.chalk.blue;
    const gray = chalk_cjs_1.chalk.gray;
    const c = chalk_cjs_1.chalk.cyan;
    const bold = chalk_cjs_1.chalk.bold;
...
${bold(b(`Grammar`))}
    ...
    ${c(`<root>`)}       = .
    ${c(`<path>`)}       = ${data_cjs_1.validPathSegmentNameRegex.toString().replace(/(^\/|\/$)/g, gray(`$1`))} [:<path>]
...
const isParseError = (value) => value instanceof Error;
...
const renderSyntaxError = (input) => {
    ...
    if (!multipleInputs) {
        // eslint-disable-next-line
        const e = getError(badOnes[0]);
        const pattern = e?.context.pattern;
        const hint = e?.context.hint ? `. ${e.context.hint}` : ``;
        message = `Your log filter's pattern${foundIn} was invalid: "${chalk_cjs_1.chalk.red(pattern)}${hint}"\n\n${(0, exports.renderSyntaxManual)()}`;
    }
        ...
        const pattern = e?.context.pattern;
        const hint = e?.context.hint ? `. ${e.context.hint}` : ``;
        message = `One of the patterns in your log filter${foundIn} was invalid: "${chalk_cjs_1.chalk.red(pattern)}"${hint}\n\n${(0, exports.renderSyntaxManual)()}`;
    }
    else {
        const patterns = badOnes
            .map((e) => {
            const hint = e.context.hint ? chalk_cjs_1.chalk.gray(`  ${e.context.hint}`) : ``;
            return `    ${chalk_cjs_1.chalk.red(e.context.pattern)}${hint}`;
        })
            .join(`\n`);
        const intro = allBad
...
const processLogFilterInput = (defaults, pattern, foundIn) => {
    const errPatterns = (0, exports.parse)(defaults, pattern);
    const goodOnes = errPatterns.filter((value) => !(value instanceof Error));
    const badOnes = errPatterns.filter((value) => value instanceof Error);
    let patterns = null;
    if (badOnes.length) {
        if (goodOnes.length) {
        ...
        const message = (0, exports.renderSyntaxError)({ errPatterns, foundIn });
        // todo use logger
