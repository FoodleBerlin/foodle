...
generatorHandler({
    ...
    async onGenerate({ dmmf, otherGenerators, generator, schemaPath }) {
         ...
         */
        if (Gentime.settings.data.output.directory !== 'default' &&
...
function getPrismaClientImportIdForItsGeneratorOutputConfig(prismaClientGeneratorConfig) {
    ...
    if (prismaClientGeneratorConfig.output.value.endsWith(prismaClientDefaultOutput)) {
         ...
         * care.
         */
        const dirPrismaClientOutputWithoutTrailingNodeModulesMoniker = resolveGitHubActionsWindowsPathTilde(expandTilde(prismaClientGeneratorConfig.output.value.replace(prismaClientDefaultOutput, '')));
        const dirProjectForThisNexusPrisma = resolveGitHubActionsWindowsPathTilde(expandTilde(Path.join(__dirname, '../../../..')));
