...
const dindist_1 = (0, tslib_1.__importDefault)(require("dindist"));
const expand_tilde_1 = (0, tslib_1.__importDefault)(require("expand-tilde"));
const Path = (0, tslib_1.__importStar)(require("path"));
const generator_1 = require("../generator");
const settingsLoader_1 = require("../generator/gentime/settingsLoader");
const settingsSingleton_1 = require("../generator/gentime/settingsSingleton");
const debugNexusPrisma_1 = require("../helpers/debugNexusPrisma");
const prismaExternalToInternalDMMF_1 = require("../helpers/prismaExternalToInternalDMMF");
...
(0, generator_helper_1.generatorHandler)({
    ...
    async onGenerate({ dmmf, otherGenerators, generator, schemaPath }) {
        ...
        if (!prismaClientGenerator) {
            ...
            throw new Error(`Nexus Prisma cannot be used without Prisma Client. Please add it to your Prisma Schema file.`);
        }
        if (generator.isCustomOutput) {
            if (!generator.output) {
                throw new Error(`Failed to read the custom output path.`);
            }
            settingsSingleton_1.Gentime.changeSettings({
                output: {
            ...
            (0, diagnostic_1.renderWarning)({
                ...
                solution: `Create a configuration file in one of the following locations:\n\n${(0, diagnostic_1.renderList)(settingsLoader_1.supportedSettingsModulePaths)}\n\nThen add the following code:\n\n${(0, diagnostic_1.renderCodeBlock)((0, dindist_1.default) `
        ...
        settingsSingleton_1.Gentime.settings.change({
            prismaClientImportId: getPrismaClientImportIdForItsGeneratorOutputConfig(prismaClientGenerator),
        });
        const internalDMMF = (0, prismaExternalToInternalDMMF_1.externalToInternalDmmf)(dmmf);
        (0, settingsLoader_1.loadUserGentimeSettings)();
        /**
         * If the output path is some explicit relative path then make it absolute relative to the Prisma Schema file directory.
         */
        if (settingsSingleton_1.Gentime.settings.data.output.directory !== 'default' &&
            !Path.isAbsolute(settingsSingleton_1.Gentime.settings.data.output.directory)) {
            settingsSingleton_1.Gentime.settings.change({
                output: {
                    directory: Path.join(Path.dirname(schemaPath), settingsSingleton_1.Gentime.settings.data.output.directory),
                },
            });
        }
        (0, generator_1.generateRuntimeAndEmit)(internalDMMF, settingsSingleton_1.Gentime.settings);
        process.stdout.write(`You can now start using Nexus Prisma in your code. Reference: https://pris.ly/d/nexus-prisma\n`);
    },
});
/**
 * Helpers
 ...
 */
function getPrismaClientImportIdForItsGeneratorOutputConfig(prismaClientGeneratorConfig) {
    ...
    if (prismaClientGeneratorConfig.output.value.endsWith(prismaClientDefaultOutput)) {
         ...
         * care.
         */
        const dirPrismaClientOutputWithoutTrailingNodeModulesMoniker = (0, utils_1.resolveGitHubActionsWindowsPathTilde)((0, expand_tilde_1.default)(prismaClientGeneratorConfig.output.value.replace(prismaClientDefaultOutput, '')));
        const dirProjectForThisNexusPrisma = (0, utils_1.resolveGitHubActionsWindowsPathTilde)((0, expand_tilde_1.default)(Path.join(__dirname, '../../../..')));
        const dirDiff = Path.relative(dirPrismaClientOutputWithoutTrailingNodeModulesMoniker, dirProjectForThisNexusPrisma);
        (0, debugNexusPrisma_1.d)(`found prisma client/nexus prisma layout:`, {
